...
async fn miner_executor_validation_flow() -> Result<()> {
    ...
    for executor in &miner.executors {
        info!("  - Executor {}: {} ({}GB)", executor.id, executor.gpu_model, executor.vram_gb);
    }
    ...
    info!("Validator: Discovering executors from miner {}...", miner.id);
    ...
    info!("Validator: Selected executor {} for validation", selected_executor.id);
    ...
    info!("Validator: Generating challenge for executor {}...", selected_executor.id);
    let challenge_params = format!(
        "{{\"gpu_model\":\"{}\",\"vram_gb\":{},\"seed\":12345}}",
        selected_executor.gpu_model,
        selected_executor.vram_gb
    );
    
    info!("Validator: Sending challenge to executor...");
    sleep(Duration::from_millis(100)).await; // Simulate network delay
    
    info!("Executor {}: Received challenge, processing...", selected_executor.id);
    sleep(Duration::from_secs(1)).await; // Simulate GPU computation
    
    info!("Executor {}: Challenge completed, sending result...", selected_executor.id);
    sleep(Duration::from_millis(100)).await; // Simulate network delay
    
    info!("Validator: Received result, verifying...");
    sleep(Duration::from_millis(500)).await; // Simulate verification
    
    info!("Validator: ✅ Verification PASSED for executor {}", selected_executor.id);
