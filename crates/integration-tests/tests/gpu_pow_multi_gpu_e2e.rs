//! Multi-GPU Proof-of-Work End-to-End Integration Test
//!
//! This test simulates the complete flow with multi-GPU support:
//! 1. A miner with multiple GPUs
//! 2. A validator issuing a multi-GPU PoW challenge
//! 3. The executor completing the challenge across all GPUs
...
// Custom struct that matches gpu-attestor JSON output with multi-GPU support
...
/// Detect GPUs using the new multi-GPU detector
async fn detect_gpus(config: &TestConfig) -> Result<usize> {
    info!("Detecting GPUs on the system using unified multi-GPU detector...");
    ...
    for line in stdout.lines() {
        if line.contains("Total GPUs detected:") {
            gpu_count = line.split(':').last()
                .and_then(|s| s.trim().parse::<usize>().ok())
                .unwrap_or(0);
        } else if line.contains("Total Cores:") && !line.contains("per SM") {
            total_cores = line.split(':').last()
                .and_then(|s| s.trim().parse::<usize>().ok())
                .unwrap_or(0);
        } else if line.contains("Total Memory:") {
            if let Some(mem_str) = line.split(':').last() {
                total_memory_gb = mem_str.trim()
                    .split_whitespace()
                    .next()
    ...
    info!("Multi-GPU Detection Summary:");
...
/// Simulate a miner's multi-GPU executor
async fn simulate_multi_gpu_miner_executor(
    config: &TestConfig,
    ...
    if let Ok(metadata) = serde_json::from_str::<MultiGpuMetadata>(&gpu_result.metadata_json) {
        if metadata.multi_gpu {
            ...
            info!("  - Synchronization: {}", if metadata.synchronization_success { "PASSED" } else { "FAILED" });
            info!("  - Anti-spoofing: {}", if metadata.anti_spoofing_passed { "PASSED" } else { "FAILED" });
            info!("  - Core saturation: {}", if metadata.core_saturation_validated { "PASSED" } else { "FAILED" });
            ...
            for device in &metadata.devices {
                ...
                info!("    - Bandwidth utilization: {:.1}%", device.bandwidth_utilization);
                info!("    - Compute throughput: {:.1} TFLOPS", device.compute_tflops);
            }
        }
    }
...
/// The main multi-GPU end-to-end test
#[tokio::test]
async fn multi_gpu_pow_end_to_end_flow() -> Result<()> {
    // Initialize logging
    ...
    info!("=== Multi-GPU PoW End-to-End Test Starting ===");
    ...
    if detected_gpu_count != config.expected_gpu_count {
        info!("Note: Expected {} GPUs but detected {}. Tests will use detected count.", 
    ...
    // Step 2: Generate challenge appropriate for multi-GPU
    info!("\n--- Step 2: Challenge Generation ---");
    
    // Create a challenge that will use all detected GPUs
    let challenge = if detected_gpu_count > 1 {
        // Multi-GPU challenge: smaller per-GPU memory requirement
        ChallengeParameters {
            challenge_type: "matrix_multiplication_pow".to_string(),
            parameters_json: String::new(),
            expected_duration_seconds: 10,
            difficulty_level: 5,
            seed: "multi_gpu_seed".to_string(),
            machine_info: None,
            gpu_pow_seed: 12345678,
            matrix_dim: 512,  // Smaller matrices for multi-GPU
            num_matrices: 2048 * detected_gpu_count as u32,  // Scale with GPU count
            matrix_a_index: 0,
            matrix_b_index: 1,
            validator_nonce: "multi_gpu_e2e_test_nonce_42".to_string(),
            num_iterations: 2000,
            verification_sample_rate: 0.1,
        }
    } else {
        // Single GPU challenge (fallback)
        let challenge_gen = ChallengeGenerator::new();
        challenge_gen.generate_challenge(
            &config.validator_gpu_model,
            config.validator_vram_gb,
            Some("multi_gpu_e2e_test_nonce_42".to_string()),
        )?
    };
    ...
    // Step 3: Miner executes challenge on multiple GPUs
    info!("\n--- Step 3: Multi-GPU Miner Execution ---");
    info!("Miner: Executing on {} GPU(s)...", detected_gpu_count);
    let miner_result = simulate_multi_gpu_miner_executor(&config, &challenge).await?;
    ...
    info!("Verification completed in {:.2} ms", verification_time.as_millis());
    ...
    // Parse metadata for multi-GPU validation
    if let Ok(metadata) = serde_json::from_str::<MultiGpuMetadata>(&miner_result.metadata_json) {
        if metadata.multi_gpu {
            assert!(metadata.synchronization_success, "Multi-GPU synchronization failed");
            assert!(metadata.anti_spoofing_passed, "Anti-spoofing validation failed");
            assert!(metadata.core_saturation_validated, "Core saturation validation failed");
    ...
    assert!(
        ...
        "Execution/Verification ratio should be reasonable (got {:.1}x)",
        execution_to_verification_ratio
    );

    info!("\n=== Multi-GPU PoW End-to-End Test PASSED ===");

    Ok(())
}

/// Test true multi-GPU execution
#[tokio::test]
async fn test_true_multi_gpu_execution() -> Result<()> {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("info")
        .with_test_writer()
        .try_init();

    info!("=== True Multi-GPU Execution Test ===");
    
    let config = TestConfig::new()?;
    
    // Step 1: Detect GPUs
    info!("\n--- Step 1: Multi-GPU Detection ---");
    let gpu_count = detect_gpus(&config).await?;
    
    if gpu_count < 2 {
        warn!("Test requires at least 2 GPUs, found {}. Skipping.", gpu_count);
        return Ok(());
    }
    
    info!("Detected {} GPUs, will use all for testing", gpu_count);
    
    // Step 2: Create a smaller challenge suitable for multi-GPU distribution
    info!("\n--- Step 2: Creating Multi-GPU Challenge ---");
    ...
    info!("  - Total matrices: {} ({} per GPU)", challenge.num_matrices, challenge.num_matrices / gpu_count as u32);
    info!("  - Iterations: {}", challenge.num_iterations);
    
    // Step 3: Execute on multi-GPU
    ...
    assert_eq!(metadata.device_count, gpu_count, "Should use all available GPUs");
    assert!(metadata.multi_gpu, "Should be marked as multi-GPU execution");
    
    info!("\n✅ Multi-GPU execution successful!");
    info!("  - Used {} GPUs", metadata.device_count);
    info!("  - Total execution time: {} ms", miner_result.execution_time_ms);
    
    for device in &metadata.devices {
        info!("\n  GPU {} performance:", device.device_id);
        info!("    - Execution time: {} ms", device.execution_time_ms);
        info!("    - Bandwidth: {:.1}%", device.bandwidth_utilization);
        assert!(device.bandwidth_utilization > 0.0 && device.bandwidth_utilization <= 100.0, 
