...
impl GpuValidatorV2 {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        let attestation_json = String::from_utf8(output.stdout)
            .context("Invalid UTF-8 in gpu-attestor output")?;
    ...
    pub async fn verify_challenge_sampled(
        ...
        let validator_gpu = self.validator_gpu_model.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Validator GPU not initialized"))?;
        
        info!("Verifying GPU PoW challenge with sampling for GPU: {}", result.gpu_model);
        ...
        info!(
            ...
            num_samples, total_iterations, sample_rate * 100.0
        );
        ...
        info!("Executing {} sampled iterations locally on {}", num_samples, validator_gpu);
        ...
        // Parse local result
        let local_result_json = String::from_utf8(output.stdout)
            .context("Invalid UTF-8 in gpu-attestor output")?;
        ...
        debug!(
            ...
            local_execution_time.as_millis(), expected_full_time, reported_time_ms
        );
