...
impl WeightAllocationEngine {
    ...
    pub fn calculate_weight_distribution(
        ...
        let burn_weight = burn_allocation.as_ref().map(|b| b.weight as u64).unwrap_or(0);
        ...
        for (category, miners) in filtered_miners {
            ...
            let category_weights = self.distribute_category_weight(&miners, category_weight_pool)?;
            
            // Calculate category statistics
            let total_score: f64 = miners.iter().map(|(_, score)| score).sum();
            let allocation_percentage = (category_weight_pool as f64 / remaining_weight as f64) * 100.0;
        ...
        let miners_served = all_weights.len() as u32 - if burn_allocation.is_some() { 1 } else { 0 };
    ...
    fn calculate_burn_allocation(&self, total_weight: u64) -> Result<Option<BurnAllocation>> {
        ...
        let burn_weight = (total_weight as f64 * self.emission_config.burn_percentage / 100.0) as u16;
    ...
    fn calculate_category_pools(
        ...
        for category in filtered_miners.keys() {
            if let Some(allocation_percentage) = self.emission_config.get_gpu_allocation(category) {
                let weight_pool = (total_remaining_weight as f64 * allocation_percentage / 100.0) as u64;
...
mod tests {
    ...
    fn create_test_miners() -> HashMap<String, Vec<(MinerUid, f64)>> {
        ...
        miners.insert("H200".to_string(), vec![
            (MinerUid::new(4), 0.9),
            (MinerUid::new(5), 0.7),
        ]);
    ...
    fn test_allocation_validation() {
        ...
        let valid_weights = vec![
            NormalizedWeight { uid: 1, weight: 1000 },
            NormalizedWeight { uid: 2, weight: 2000 },
        ];
        ...
        let duplicate_weights = vec![
            NormalizedWeight { uid: 1, weight: 1000 },
            NormalizedWeight { uid: 1, weight: 2000 },
        ];
    ...
    fn test_edge_cases() {
        ...
        let miners = create_test_miners();
        let distribution = engine_high_min.calculate_weight_distribution(miners).unwrap();
