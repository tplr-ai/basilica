...
pub async fn handle_assignment_command(
    ...
    match command {
        AssignmentCommand::Assign {
            ...
            info!("Assigning executor {} to validator {}", executor_id, validator_hotkey);
            
            assignment_db
                .create_assignment(executor_id, validator_hotkey, "cli-user", notes.as_deref())
                .await?;
            
            println!("✓ Successfully assigned executor {} to validator {}", executor_id, validator_hotkey);
        ...
        AssignmentCommand::Unassign { executor_id } => {
            ...
            if let Some(assignment) = assignment_db.get_assignment_by_executor(executor_id).await? {
                assignment_db.delete_assignment(executor_id, "cli-user").await?;
                println!("✓ Successfully unassigned executor {} from validator {}", 
                    executor_id, assignment.validator_hotkey);
            } else {
                return Err(anyhow!("Executor {} is not currently assigned", executor_id));
            }
        }
        ...
        AssignmentCommand::List { validator } => {
            if let Some(validator_hotkey) = validator {
                ...
                let assignments = assignment_db.get_assignments_for_validator(validator_hotkey).await?;
                
                if assignments.is_empty() {
                    println!("No executors assigned to validator {}", validator_hotkey);
                } else {
                    println!("Executors assigned to validator {}:", validator_hotkey);
                    println!("{:<20} {:<25} {}", "Executor ID", "Assigned At", "Notes");
                    println!("{}", "-".repeat(70));
                ...
                if assignments.is_empty() {
                    ...
                    println!("{:<20} {:<50} {:<25} {}", 
                        "Executor ID", "Validator Hotkey", "Assigned At", "Notes");
                    println!("{}", "-".repeat(120));
        ...
        AssignmentCommand::Coverage => {
            ...
            println!("Unassigned Executors:  {}", stats.total_executors - stats.assigned_executors);
            println!("Covered Stake:         {:.2}%", stats.covered_stake_percentage);
            println!("Covered Validators:    {}/{}", stats.covered_validators, stats.total_validators);
        ...
        AssignmentCommand::Suggest { min_coverage } => {
            ...
            let suggestions = suggester.suggest_assignments(executors, stakes, assignments).await?;
            ...
            if suggestions.is_empty() {
                ...
                println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
                println!("{:<10} {:<20} {:<50} {}", "Priority", "Executor", "Validator", "Reason");
                println!("{}", "-".repeat(140));
                
                for suggestion in suggestions {
                    let priority_str = match suggestion.priority {
                        crate::services::assignment_manager::AssignmentPriority::Critical => "🔴 CRITICAL",
        ...
        AssignmentCommand::History { executor_id, limit } => {
            if let Some(executor_id) = executor_id {
                ...
                if history.is_empty() {
                    ...
                    println!("{:<15} {:<50} {:<25} {}", "Action", "Validator", "Performed At", "By");
                    println!("{}", "-".repeat(110));
                ...
                if history.is_empty() {
                    ...
                    println!("{:<20} {:<15} {:<50} {:<25} {}", 
                        "Executor", "Action", "Validator", "Performed At", "By");
                    println!("{}", "-".repeat(130));
        ...
        AssignmentCommand::Stakes { min_stake } => {
            ...
            if filtered_stakes.is_empty() {
                println!("No validators found with minimum stake of {:.2} TAO", min_stake.unwrap_or(0.0));
            } else {
                println!("Validator Stakes{}:", 
                    min_stake.map_or(String::new(), |min| format!(" (≥{:.2} TAO)", min)));
                println!("{:<50} {:<15} {:<15} {}", "Validator Hotkey", "Stake (TAO)", "Percentage", "Last Updated");
                println!("{}", "-".repeat(100));
                
                for stake in filtered_stakes {
                    println!("{:<50} {:<15.2} {:<15.2}% {}", 
        ...
        AssignmentCommand::Import { path, dry_run } => {
            ...
            for assignment in assignments {
                if let (Some(executor_id), Some(validator_hotkey)) = (
                    ...
                    if *dry_run {
                        ...
                        match assignment_db.create_assignment(executor_id, validator_hotkey, "import", notes).await {
                            Ok(_) => imported += 1,
                            Err(e) => {
                                error!("Failed to import assignment {}->{}: {}", executor_id, validator_hotkey, e);
                                errors += 1;
                            }
                        }
                    }
            ...
            if *dry_run {
                println!("Dry run complete: {} assignments would be imported, {} errors", imported, errors);
            } else {
                println!("Import complete: {} assignments imported, {} errors", imported, errors);
            }
        }
    }
...
mod tests {
    ...
    async fn test_assign_command() -> Result<()> {
        ...
        assignment_db.update_validator_stake("test-validator", 1000.0, 50.0).await?;
        
        // Test assignment creation directly through the database
        // (CLI command test would require mocking the database connection)
        assignment_db.create_assignment("test-exec-1", "test-validator", "cli-user", Some("Test assignment")).await?;
        
        // Verify assignment was created
        let assignment = assignment_db.get_assignment_by_executor("test-exec-1").await?;
    ...
    async fn test_unassign_command() -> Result<()> {
        ...
        assignment_db.create_assignment("test-exec-1", "test-validator", "test", None).await?;
        
        // Test assignment deletion directly
        assignment_db.delete_assignment("test-exec-1", "cli-user").await?;
    ...
    async fn test_list_command_all() -> Result<()> {
        ...
        assignment_db.create_assignment("test-exec-1", "validator-1", "test", None).await?;
        assignment_db.create_assignment("test-exec-2", "validator-2", "test", Some("Test note")).await?;
        
        // Test listing all assignments
        let assignments = assignment_db.get_all_assignments().await?;
        assert_eq!(assignments.len(), 2);
        
        let exec1_assignment = assignments.iter().find(|a| a.executor_id == "test-exec-1").unwrap();
        assert_eq!(exec1_assignment.validator_hotkey, "validator-1");
        assert_eq!(exec1_assignment.notes, None);
        
        let exec2_assignment = assignments.iter().find(|a| a.executor_id == "test-exec-2").unwrap();
    ...
    async fn test_coverage_command() -> Result<()> {
        ...
        assignment_db.update_validator_stake("validator-1", 1000.0, 60.0).await?;
        assignment_db.update_validator_stake("validator-2", 500.0, 40.0).await?;
    ...
    async fn test_suggest_command() -> Result<()> {
        ...
        // Add validator stakes
        assignment_db.update_validator_stake("validator-1", 1000.0, 40.0).await?;
        assignment_db.update_validator_stake("validator-2", 800.0, 35.0).await?;
        assignment_db.update_validator_stake("validator-3", 500.0, 25.0).await?;
    ...
    async fn test_history_command() -> Result<()> {
        ...
        assignment_db.create_assignment("test-exec-1", "validator-1", "test", None).await?;
        assignment_db.delete_assignment("test-exec-1", "test").await?;
