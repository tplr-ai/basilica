...
def main():
    ...
    rental = client.start_cpu_rental(offering_id=offering.id, name="example-cpu")
    ...
    try:
        ...
        print(f"To manually stop later, run: client.stop_cpu_rental('{rental.name}')")
        return

    # Step 7: Stop the rental
    print("\n[Step 7] Stopping rental...")
    try:
        result = client.stop_cpu_rental(rental.name)
        print("\n" + "=" * 60)
        print("  RENTAL STOPPED")
        print("=" * 60)
        print(f"\n  Name: {result.name}")
