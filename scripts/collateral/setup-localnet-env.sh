...
MINER_PUBKEY_FILE="${MINER_PUBKEY_FILE:-${LOCALNET_WALLETS_DIR}/miner_1/hotkeys/defaultpub.txt}"
...
miner_address=""
miner_private_key=""
miner_wallet_file_used=""
miner_wallet_file_created=false
...
load_or_create_miner_wallet() {
  local wallet_json
  local current_umask

  if [[ -f "$MINER_WALLET_FILE" ]]; then
    miner_private_key="$(read_wallet_var "$MINER_WALLET_FILE" "MINER_PRIVATE_KEY")"
    miner_address="$(read_wallet_var "$MINER_WALLET_FILE" "MINER_ADDRESS")"
    [[ -n "$miner_private_key" ]] || die "MINER_PRIVATE_KEY missing in ${MINER_WALLET_FILE}"
    miner_private_key="$(normalize_private_key "$miner_private_key")"
    if [[ -z "$miner_address" ]]; then
      miner_address="$(cast wallet address --private-key "$miner_private_key")"
    fi
    miner_wallet_file_used="$MINER_WALLET_FILE"
    return
  fi

  mkdir -p "$(dirname "$MINER_WALLET_FILE")"

  wallet_json="$(cast wallet new --json)"
  miner_private_key="$(printf '%s\n' "$wallet_json" | awk -F'"' '/"private_key"/ {print $4; exit}')"
  miner_address="$(printf '%s\n' "$wallet_json" | awk -F'"' '/"address"/ {print $4; exit}')"

  [[ -n "$miner_private_key" ]] || die "failed to parse miner private key from cast wallet output"
  [[ -n "$miner_address" ]] || die "failed to parse miner address from cast wallet output"
  ...
  current_umask="$(umask)"
  umask 077
  cat > "$MINER_WALLET_FILE" <<EOF
# Generated by scripts/collateral/setup-localnet-env.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
MINER_ADDRESS=$miner_address
MINER_PRIVATE_KEY=$miner_private_key
EOF
  umask "$current_umask"
  miner_wallet_file_created=true
}
...
load_hotkey_defaults() {
  ...
  if [[ -n "$MINER_PUBKEY" ]]; then
    ...
    [[ -f "$MINER_PUBKEY_FILE" ]] || die "miner hotkey file not found: ${MINER_PUBKEY_FILE}"
    MINER_PUBKEY="$(python3 -c "import json,sys; print(json.load(sys.stdin)['publicKey'])" < "$MINER_PUBKEY_FILE")"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --miner-pubkey)
      shift
      [[ $# -gt 0 ]] || die "--miner-pubkey requires a value"
      MINER_PUBKEY="$1"
      ;;
    --miner-pubkey-file)
      shift
...
BASILICA_COLLATERAL_NETWORK=local
BASILICA_COLLATERAL_CONTRACT_ADDRESS=$proxy_address

# Local signer key for collateral tx scripts (localnet only)
PRIVATE_KEY=$PRIVATE_KEY

# Pre-generated miner wallet (localnet only)
MINER_PRIVATE_KEY=$miner_private_key
MINER_ADDRESS=$miner_address
...
if [[ -n "$miner_wallet_file_used" ]]; then
  if [[ "$miner_wallet_file_created" == true ]]; then
    echo "Miner wallet file created:    $miner_wallet_file_used"
  else
    echo "Miner wallet file reused:     $miner_wallet_file_used"
  fi
fi
echo
echo "Next:"
echo "  source \"$ENV_FILE\""
echo "  basilica collateral query trustee"
echo "  basilica collateral tx deposit --private-key \"\$PRIVATE_KEY\" --hotkey \"\$MINER_PUBKEY\" --node-id \"\$NODE_ID\" --alpha-amount \"\$ALPHA_AMOUNT_RAO\""
