...
SOURCE_IMAGE="basilica/payments"
TARGET_IMAGE="ghcr.io/one-covenant/basilica/payments"
IMAGE_TAG="latest"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  --source-image SOURCE     Source Docker image (default: basilica/payments)"
            echo "  --target-image TARGET     Target Docker image (default: ghcr.io/one-covenant/basilica/payments)"
...
echo "Tagging and pushing payments image..."
...
if ! docker images --format "table {{.Repository}}:{{.Tag}}" | grep -q "^${SOURCE_IMAGE_FULL}$"; then
    ...
    echo "Please build the image first using: ./scripts/payments/build.sh"
    exit 1
fi
